...
TEST(DualConditionedList, PushCondWait)
{
...
TEST(ConditionedListPair, PushSizeAndPop)
{
  conditioned_list_pair<int, std::string> int_and_strings_list;
  ...
  EXPECT_EQ((conditioned_list_pair<int, std::string>::typeType::typeT1),
  ...
  EXPECT_EQ((conditioned_list_pair<int, std::string>::typeType::typeT2),
            int_and_strings_list.front_type ());
  {
...
TEST(ConditionedListPair, Asynchronously)
{
  auto seq_int_producer = [] (conditioned_list_pair<std::string, int>* sl, std::list<int>* il)
  {
  ...
  auto seq_string_producer = [] (conditioned_list_pair<std::string, int>* sl, std::list<std::string>* il)
  {
    for (auto it = il->begin(); it != il->end(); ++it)
    {
      sl->push_back (*it);
  ...
  conditioned_list_pair<std::string, int> strings_and_ints_list;
  ...
  while (!string_producer_term || !int_producer_term)
  {
    ...
    if (strings_and_ints_list.front_type ()
        == conditioned_list_pair<std::string, int>::typeType::typeT1)
    {
    ...
    if (strings_and_ints_list.front_type ()
        == conditioned_list_pair<std::string, int>::typeType::typeT2)
    {
